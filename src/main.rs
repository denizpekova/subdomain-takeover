...
async fn handle_takeover(stdin: &mut io::BufReader<io::Stdin>) -> Result<()> {
    let target = get_input(&format!("{} ", "Hedef Domain (örn: sub.example.com):".bold().green()), stdin).await?;
    if !target.is_empty() {
...
async fn handle_port_scan(stdin: &mut io::BufReader<io::Stdin>) -> Result<()> {
    let target = get_input(&format!("{} ", "Hedef Adres (örn: example.com):".bold().green()), stdin).await?;
    if target.is_empty() {
...
async fn handle_subdomain_discovery(stdin: &mut io::BufReader<io::Stdin>) -> Result<()> {
    let target = get_input(&format!("{} ", "Hedef Domain (örn: example.com):".bold().green()), stdin).await?;
    if target.is_empty() {
    ...
    let wordlist_source = match wl_choice.as_str() {
        ...
        "2" => {
            let wordlist = get_input(&format!("{} ", "Wordlist Dosya Yolu (örn: subdomains.txt):".bold().green()), stdin).await?;
